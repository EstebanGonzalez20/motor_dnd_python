...
class CombatSystem:
    ...
    def attack(self, attacker: Entity, target: Entity, damage_roll) -> CombatResult:
        ...
        if not self._action_system.can_perform(attacker, ActionType.ACTION):
        ...
        self._action_system.spend(attacker, ActionType.ACTION)
 
        result: AttackResult = self._action_system.attack_roll(attacker, target)
